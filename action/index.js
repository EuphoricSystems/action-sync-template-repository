...
const allowedStrategy = ["pull_request", "push"];
...
if (!allowed.includes(github.context.eventName)) {
  core.warning(
    `action ran on incompatible event "${
...
if (!allowedStrategy.includes(inputs.updateStrategy)) {
  core.warning(
    `incompatible updated strategy "${
      inputs.updateStrategy
    }", only "${allowedStrategy.join('", "')}" are allowed`
  );
  process.exit(0);
}
